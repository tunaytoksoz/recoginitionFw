...
//
//  Created by Tunay Toksöz on 2.05.2023.
//
//
...
public class IbanController: UIViewController{
    
   public let ibanValidator =  IbanValidator()
    
   public weak var delegate : IbanControllerDelegate?
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            libraryButton.widthAnchor.constraint(equalToConstant: 120120),
...
extension IbanController {
    ...
    func textRecognition(){
        ...
        request = VNRecognizeTextRequest(completionHandler: { (request, error) in
            ...
            if result.isEmpty {
                self.delegate?.didCompletedRecognition(iban: "Nil", isSucces: false)
            } else {
                self.delegate?.didCompletedRecognition(iban: result.first!, isSucces: true)
            }
...
extension IbanController : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
