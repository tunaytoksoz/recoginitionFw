...
public class CustomController: UIViewController{
    ...
    public weak var delegate : CustomControllerDelegate?
    
    public init(regex: String) {
        customValidator = CustomValidator(regex: regex)
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            libraryButton.bottomAnchor.constraint(equalTo: stackView.bottomAnchor, constant: 5),
...
extension CustomController {
    ...
    func textRecognition(){
        ...
        request = VNRecognizeTextRequest(completionHandler: { (request, error) in
            ...
            if result.isEmpty {
                self.delegate?.didCompletedRecognition(result: "", isSucces: false)
            } else {
                guard let str = result.first else { return }
                self.delegate?.didCompletedRecognition(result: str , isSucces: true)
            }
